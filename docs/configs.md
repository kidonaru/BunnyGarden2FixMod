...
| `TopsSkinMinOffset` | `0.007` | 上衣 肌からの最小距離 (m)<br>donor 服が donor 側 Babydoll 肌にめり込んでいる箇所を解決するための safety margin（メートル）。<br>donor 服頂点 v_i の signedD(v_i, donor_skin) < TopsSkinMinOffset なら、近傍 donor skin 頂点を<br>逆距離² 重み平均で内側へ凹ませ、d_donor を最低この値まで持ち上げます。その後で標準の距離保存<br>(target skin との差分を target 法線方向に push) を適用します。<br>donor 服頂点自体は触らず、reference skin だけを部分的に凹ませる方式のため、頂点間の push 量分散<br>による裏面 flip が原理的に起きません。<br>0 で無効（純粋な距離保存のみ＝めり込みは未補正で持ち越し）。 |
| `TopsSkinSampleRadius` | `0.06` | 上衣 肌サンプル半径 (m)<br>上衣移植の距離保存で、donor 頂点が参照する skin の最近傍点を「半径内の全 skin 頂点」<br>の距離重み平均で求めるようにします。0 で K=3 固定、>0 で指定半径内の全頂点を<br>使った滑らかな表面推定になります。半径内に頂点が無い場合は K=3 にフォールバック。<br>skin の細かいトポロジ凹凸 (鎖骨・肩甲骨・肩関節等) で d_donor / d_target が<br>不安定にばらつく場合に有効です。 |
...
| `TopsSkinShrink` | `0` | 上衣 肌押込み量 (m)<br>上衣移植時、target の mesh_skin_upper の頂点を「donor tops 表面より内側」に<br>保つ目標距離（メートル）。tops と肌が z-fighting している箇所では、まず肌を<br>tops 表面まで引っ込めてから、さらにこの距離だけ内側に押し込みます。<br>StockingSkinShrink の上半身版。distance preservation (donor cloth 補正) と<br>補正対象が disjoint (cloth vs skin) なので併用可能、機能重複しません。<br>0 で無効化。 |
| `TopsSkinShrinkFalloffRadius` | `0.001` | 上衣 肌押込み フェード半径 (m)<br>肌の押し込み量を、隣接 mesh（mesh_skin_lower 等）からの距離で線形フェードさせる半径（メートル）。<br>距離 0 で押し込み 0、半径以上で 100%。境界（首・ウエスト等）での段差を防ぎます。0 で無効化（一様押し込み）。 |
| `TopsSkinShrinkSampleRadius` | `0` | 上衣 肌押込み cloth サンプル半径 (m)<br>肌押込み (TopsSkinShrink) 計算で、各 skin 頂点が参照する cloth の最近傍点を「半径内の全 cloth 頂点」<br>の距離重み平均で求めます。0 で K=3 固定（既定）、>0 で指定半径内の全頂点を使った滑らかな表面推定に<br>なります。半径内に頂点が無い場合は K=3 にフォールバック。<br>mesh_costume の frill / sleeve / 双面ジオメトリで signedD が頂点単位で揺れて push 量が不安定な場合に有効。 |
| `TopsLoaderVerbose` | `false` | 上衣移植デバッグログ<br>上衣移植 (Tops Transplant) の詳細ログを出力します。dev 向け調査用。<br>donor / target の SMR 一覧、Tops 候補、同名重複、共通/donor 専用/target 専用 SMR の差分を<br>BepInEx ログに出力します。F9 設定パネルには出ません。 |
...
| キー | デフォルト | 説明 |
|------|-----------|------|
| `SwimWearPhysicsDiag` | `false` | 水着 物理診断ログ<br>DIAGNOSTIC ONLY — issue 解決後削除予定。<br>SwimWear setup 時の hierarchy / animator / MagicaCloth を初回 (char, costume, scene) ごとに 1 回だけログ出力します。 |
