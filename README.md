...
- F5キーでON/OFF、F6キーでカメラの固定ができるフリーカメラ機能。コントローラー操作にも対応。
...
| `ControllerToggleFixedFreeCam` | `X` | フリーカメラ固定 ON/OFF に使うボタン |
...
コントローラーの既定操作は **Select + Y** で ON/OFF、**Select + X** で固定切り替えです。
...
> **注意**: フリーカメラ中は誤操作を避けるため、既定ではゲーム本体の UI を自動で隠します。終了確認や確認ダイアログが表示された場合は、自動的に UI が復帰し、終了確認ではフリーカメラも自動解除されます。
